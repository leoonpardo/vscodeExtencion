...
let palabraBuscar = ''
let relative_number = '5'
let switch_split = 2
let my_numero = 1
function colorCursor(color : string){
...
export function activate(context: vscode.ExtensionContext) {
  ...
  editor.edit(async editBuilder => {
    ...
    vscode.window.showInformationMessage(`No se encontró (insensible a mayúsculas): "${targetRaw}".`);
			...
			myvar = 0;
		}else if (myvar === 5 && buscar_switch === 3) {
    const doc = editor.document;
    ...
    return;
        ...
        if (actions.length === 0) return;
        relative_number = actions[0].inner
			let num1: number = Number(relative_number);
		...
		// Mover 4 líneas hacia abajo (puedes cambiar a -4 para arriba)
		const newPosition = position.with(position.line - num1, position.character);
	...
	const uCommand = vscode.commands.registerCommand("myModal.uCommand", () => {
		...
		if (!editor) return;
			  if (myvar === 0 && buscar_switch === 1){
					...
					vscode.env.clipboard.writeText(folderPath)
				    vscode.commands.executeCommand('workbench.action.terminal.toggleTerminal');
				}
	...
	const editor = vscode.window.activeTextEditor;
      ...
      if (!wordRange) {
        vscode.window.showInformationMessage("No estás sobre una palabra.");
        return;
      }

      const wordText = document.getText(wordRange);
      const oldWord = palabraBuscar
      const newWord = await vscode.env.clipboard.readText();

      const cursorOffset =
        document.offsetAt(position) -
        document.offsetAt(wordRange.start);

      const index = wordText.indexOf(oldWord);
      if (index === -1) {
        vscode.window.showInformationMessage(
          "La palabra no contiene la subpalabra guardada."
        );
        return;
      }

      const startIndex = index;
      const endIndex = index + oldWord.length;

      const isAtStart = cursorOffset === startIndex;
      const isAtEnd = cursorOffset === endIndex;

      if (!isAtStart && !isAtEnd) {
        vscode.window.showInformationMessage(
          "El cursor no está en el inicio ni final de la subpalabra."
        );
        return;
      }

      const replaced =
        wordText.slice(0, startIndex) +
        newWord +
        wordText.slice(endIndex);

      editor.edit(editBuilder => {
        editBuilder.replace(wordRange, replaced);
      });
    ...
    return;
		...
		const end_line = start_line + num1
		const start_pos = new vscode.Position(start_line , 0)
		const end_pos = new vscode.Position(end_line, editor.document.lineAt(end_line).text.length)
    ...
    return;
		...
		const position = editor.selection.active;
		const start_line = position.line
		const end_line = start_line - num1 -1
